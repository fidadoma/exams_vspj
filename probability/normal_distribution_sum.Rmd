...
df_assignmnent <- get_prob_var(distr_type = "normal", curr_theme = curr_theme,norm_type = "sum")
df_vardesc <- df_assignmnent$var_desc %>% unnest(cols = c(data))
text_settings <- sample(df_assignmnent$settings,1)
text_question <- sample(df_assignmnent$question_sum,1)
...
k <- rnorm(1, mu_param, sigma_param) %>% round()
...
if(is.na(k_text_tmp)) {
  k_text <- sprintf("%s %d", quantifier, k*n)
} else {
  k_text <- sprintf("%s %d %s", quantifier, k*n, k_text_tmp)
}
...
l <- probability_normal_setting_sum(k, n, mu_param, sigma_param, quantifier)
solution <- l$solution
