---
title: "Range estimation"
author: "Filip Děchtěrenko"
date: '`r format(Sys.time(), "%d %B, %Y")`'
output: html_document
---

```{r setup, code = readLines("D:/Documents/git/exams_vspj/common_chunks.R")}

```


```{r data generation, echo = FALSE}

df_assignmnent <- get_var_desc()
df_vardesc <- df_assignmnent$var_desc %>% unnest(cols = c(data))
text_question <- sample(df_assignmnent$question,1)

n <- sample(c(36,42,48),1)
r <- runif(1,-1,1)

df_currvars <- sample_vars(df_vardesc, nvar = 2)

# generate X,Y data
df_data <- safe_generate_correlated_data(n,r,df_currvars)
mm <- df_data[[df_currvars$name[1]]] 
mm <- mm %>% matrix(nrow = n/6) %>% round(df_currvars$round_val[1])

interval_range <- sample(c(95,90,99),1)

u <- qt(1-(1-interval_range/100)/2, n-1) %>% round(2)
s <- sd(mm) %>% round(2)
m <- mean(mm) %>% round(2)
LB <- m-u*s/sqrt(n) %>% round(1)
UB <- m+u*s/sqrt(n) %>% round(1)

if(runif(1) < 0.5) {
  x <- LB
  interval_type <- "dolní"
} else {
  x <- UB
  interval_type <- "horní"
}
```



Question
========
```{r, results='asis'}
cat(eval(parse(text=text_question)))
```

Když se zaměříme pouze na proměnnou `r df_currvars$name[1]`, vypadají data pro tento výběr data následovně:

```{r graf,echo = FALSE, warning =F,message=F}
mm %>% knitr::kable()
```

Spočítejte interval, ve kterém se nachází s pravděpodobností `r interval_range`% průměr celé populace (konfidenční interval). Pro výsledek zadejte pouze `r interval_type` hranici intervalu a zaokrouhlete na 1 desetinné místo.


Solution
========
Pro výpočet `r interval_range`% intervalu spolehlivosti pro průměr využijeme vzorce :
$$ 
\begin{aligned}
\left[\bar{y} \, - \, t(n-1)_{1-\frac{\alpha}{2}}\sqrt{\frac{s^2}{n}}, \; 
  \bar{y} \, + \, t(n-1)_{1-\frac{\alpha}{2}}\sqrt{\frac{s^2}{n}}\right] 
\end{aligned}
$$

kde v tomto případě je $\bar{y}$=`r m`, $t(n-1)_{1-\frac{\alpha}{2}}$=`r u`, n = `r n` a s = `r s`. Interval tedy je ve tvaru `r sprintf("[%.1f,%.1f]", LB, UB)`

Answerlist
----------
* Dolní limit je $`r LB %>% exams::fmt(digits = 1)   `$.
* Horní limit je $`r UB %>% exams::fmt(digits = 1)   `$.

Meta-information
================
extype: num
exsolution: `r exams::fmt(x,digits = 1)`
exname: confidence_interval
extol: 0.1
extitle: Intervalový odhad
expoints: 1
