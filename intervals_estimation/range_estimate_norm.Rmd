...
```{r eval=in_english, results='asis'}
cat(sprintf("Compute confidence interval, in which the population mean lies with probability %d%% . Report just %s bound of the interval and round to 1 decimal place.",  interval_range,interval_type))
...
$$ 
\begin{aligned}
\left[\bar{x} \, - \, u_{1-\frac{\alpha}{2}}\sqrt{\frac{s^2}{n}}, \; 
  \bar{x} \, + \, u_{1-\frac{\alpha}{2}}\sqrt{\frac{s^2}{n}}\right] 
\end{aligned}
$$


```{r eval=in_czech, results='asis'}
cat(sprintf("kde v tomto případě je $\\bar{x}$=%.2f, $u_{1-\\frac{\\alpha}{2}}$=%.2f, n = %d a s = %.2f. Interval tedy je ve tvaru %s",  m, u, n, s,sprintf("[%.1f,%.1f]", LB, UB)))
```

```{r eval=in_english, results='asis'}
cat(sprintf("where in this case $\\bar{x}$=%.2f, $u_{1-\\frac{\\alpha}{2}}$=%.2f, n = %d and s = %.2f. Confidence interval is in form of %s",  m, u, n, s,sprintf("[%.1f,%.1f]", LB, UB)))
